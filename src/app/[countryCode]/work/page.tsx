import Image from "next/image"
import Link from "next/link"
import { caseStudies, teamMember, testimonials } from "@lib/data/case-studies"

export default function WorkPage({
  params,
}: {
  params: { countryCode: string }
}) {

  const { countryCode } = params

  return (
    <main
      className="relative overflow-hidden bg-[#f3f4f6] pt-16 pb-20 lg:pt-20 lg:pb-24"
      style={{
        fontFamily:
          '-apple-system, BlinkMacSystemFont, "SF Pro Display", "Segoe UI", sans-serif',
      }}
    >
      <div className="pointer-events-none absolute inset-0 overflow-hidden">
        <div className="absolute left-[5%] top-[8%] h-[140px] w-[420px] rounded-full bg-white/45 blur-3xl" />
        <div className="absolute right-[6%] top-[10%] h-[150px] w-[360px] rounded-full bg-white/40 blur-3xl" />
        <div className="absolute left-[18%] bottom-[8%] h-[110px] w-[420px] rounded-full bg-white/35 blur-3xl" />
      </div>

      <div className="content-container relative px-4 sm:px-6 lg:px-10">
        <div className="mx-auto max-w-[1320px]">

          {/* Heading */}

          <div className="max-w-[760px]">
            <p
              className="mb-4 text-[24px] font-bold text-[#e61e73]"
              style={{
                fontFamily: '"Comic Sans MS", "Marker Felt", cursive',
              }}
            >
              CASE STUDIES
            </p>

            <h1 className="text-[42px] font-extrabold leading-[0.95] tracking-[-0.065em] text-[#0e2547] sm:text-[56px] lg:text-[76px]">
              <span className="relative inline-block">
                Real brands. Real growth.
                <span className="absolute bottom-[8px] left-0 -z-10 h-[15px] w-[58%] bg-[#ef6a99]" />
              </span>
            </h1>
          </div>


          {/* Cards */}

          <div className="mt-14 grid gap-x-10 gap-y-16 md:grid-cols-2 xl:grid-cols-3">

            {caseStudies.map((item) => (

              <article key={item.slug} className="group">

                <Link
                  href={`/${countryCode}/work/${item.slug}`}
                  className="block"
                >

                  <div className="relative">

                    <div className="pointer-events-none absolute -top-6 right-8 z-20">

                      <svg
                        width="82"
                        height="54"
                        viewBox="0 0 82 54"
                        fill="none"
                      >
                        <path
                          d="M8 44C14 20 14 8 14 2M24 46C30 24 30 11 30 4M40 48C46 26 46 13 46 6M56 48C62 28 62 16 62 10M72 46C76 30 76 20 76 14"
                          stroke="#e61e73"
                          strokeWidth="4"
                          strokeLinecap="round"
                        />
                      </svg>

                    </div>

                    <div className="overflow-hidden bg-white shadow-[0_22px_55px_rgba(0,0,0,0.08)] transition-all duration-500 group-hover:-translate-y-2 group-hover:shadow-[0_30px_70px_rgba(0,0,0,0.12)]">

                      <div className="relative aspect-[1.05/1] w-full">

                        <Image
                          src={item.image}
                          alt={item.title}
                          fill
                          className="object-cover transition-transform duration-700 group-hover:scale-[1.035]"
                        />

                      </div>

                    </div>

                  </div>

                </Link>


                <div className="mt-8">

                  <p
                    className="text-[22px] font-bold text-[#e61e73]"
                    style={{
                      fontFamily: '"Comic Sans MS", "Marker Felt", cursive',
                    }}
                  >
                    {item.category}
                  </p>


                  <h2 className="mt-4 text-[32px] font-extrabold leading-[1.03] tracking-[-0.055em] text-[#0e2547] lg:text-[38px]">

                    <Link href={`/${countryCode}/work/${item.slug}`}>
                      {item.title}
                    </Link>

                  </h2>


                  {item.about && (
                    <p className="mt-4 text-[17px] leading-8 text-slate-500">
                      {item.about}
                    </p>
                  )}


                  <Link
                    href={`/${countryCode}/work/${item.slug}`}
                    className="mt-6 inline-flex items-center text-[15px] font-extrabold uppercase tracking-[0.02em] text-[#0e2547] transition-colors hover:text-[#e61e73]"
                  >
                    View Case Study
                  </Link>

                </div>

              </article>

            ))}

          </div>


          {/* TEAM */}

          <section className="mt-24">

            <div className="max-w-[760px]">

              <p className="mb-4 text-[24px] font-bold text-[#e61e73]">
                TEAM SECTION
              </p>

              <h2 className="text-[38px] font-extrabold text-[#0e2547]">
                The team behind your growth
              </h2>

            </div>


            <div className="mt-10 overflow-hidden rounded-[24px] bg-white shadow-[0_22px_55px_rgba(0,0,0,0.08)]">

              <div className="grid lg:grid-cols-[320px_1fr]">

                <div className="relative min-h-[340px]">

                  <Image
                    src={teamMember.image}
                    alt={teamMember.name}
                    fill
                    className="object-cover"
                  />

                </div>


                <div className="p-8">

                  <p className="text-[#e61e73] font-bold">
                    {teamMember.role}
                  </p>

                  <h3 className="text-4xl font-bold">
                    {teamMember.name}
                  </h3>

                  <p className="mt-4">
                    {teamMember.description}
                  </p>

                </div>

              </div>

            </div>

          </section>


          {/* TESTIMONIALS */}

          <section className="mt-24">

            <h2 className="text-4xl font-bold mb-10">
              What Our Clients Say
            </h2>

            <div className="grid lg:grid-cols-3 gap-8">

              {testimonials.map((item) => (

                <div
                  key={item.author}
                  className="bg-white p-8 rounded-xl shadow"
                >

                  <p>“{item.quote}”</p>

                  <p className="mt-4 font-bold">
                    — {item.author}
                  </p>

                </div>

              ))}

            </div>

          </section>

        </div>
      </div>
    </main>
  )
}